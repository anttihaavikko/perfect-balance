...
func boom(pos: Vector2):
	...
	tweener.interpolate_property(self, "size", 0, 0.6, 1.5, Tween.TRANS_BOUNCE, Tween.EASE_OUT)
	tweener.interpolate_property(self, "thickness", 0.2, 0, 1.5, Tween.TRANS_QUAD, Tween.EASE_OUT)
	tweener.interpolate_property(self, "amount", 0.02, 0, 1.5, Tween.TRANS_QUAD, Tween.EASE_IN_OUT)
	tweener.start()
